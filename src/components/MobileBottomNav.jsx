    ...
    <>
      ...
      {showMore && (
        <div className="lg:hidden fixed inset-0 z-20 bg-black/20 backdrop-blur-sm"
          ...
          <div className="fixed bottom-[64px] left-0 right-0 bg-surface border-t border-line/70 px-4 pt-4 pb-5 slide-in shadow-pop"
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 bg-surface/95 backdrop-blur border-t border-line/70 z-30">
        <ul className="flex items-end justify-around px-2 pt-2 pb-3">
          {visibleTabs.map((t) => {
            ...
            if (t.fab) {
              return (
                <li key={t.id} className="-mt-6">
                  <button onClick={() => { setShowMore(false); onFab?.(); }}
            ...
            const active = route === t.id || (t.id === 'more' && (moreRoutes.includes(route) || showMore));

            return (
              <li key={t.id}>
                <button onClick={() => handleTab(t)}
